...
/**
 * Build the list of link sources: the providers registered on the onLinkProvidersSetup
 * event, plus a built-in Media source (core com_media).
 *
 * The "editor-link-providers" key is the deprecated 6.2 name, still read so that a
 * third party plugin registering under it keeps working. To be removed in Joomla 7.0.
 *
 * @returns {Array<{key: string, title: string, icon: string, src: string, select: string}>}
 */
const getSources = () => {
  const providers = {
    ...Joomla.getOptions('editor-link-providers', {}),
    ...Joomla.getOptions('link-providers', {}),
  };
  ...
  sources.push({
    key: 'media',
    title: Joomla.Text._('JLIB_LINK_PICKER_MEDIA'),
    icon: 'pictures',
...
const openLinkPicker = () => new Promise((resolve) => {
  ...
  container.className = 'joomla-link-picker';

  const nav = document.createElement('nav');
  nav.className = 'joomla-link-picker-nav';

  const frameWrap = document.createElement('div');
  frameWrap.className = 'joomla-link-picker-frame';

  const iframe = document.createElement('iframe');
  iframe.title = Joomla.Text._('JLIB_LINK_PICKER_TITLE');
  ...
  sources.forEach((source) => {
    ...
    btn.className = 'joomla-link-picker-item';
    btn.innerHTML = `<span class="icon-${source.icon}" aria-hidden="true"></span><span class="joomla-link-picker-item-label"></span>`;
    btn.querySelector('.joomla-link-picker-item-label').textContent = source.title;
  ...
  dialog = new JoomlaDialog({
    ...
    textHeader: Joomla.Text._('JLIB_LINK_PICKER_TITLE'),
    popupButtons,
  });
  dialog.classList.add('joomla-link-picker-dialog');
...
export { openLinkPicker };
export default openLinkPicker;
